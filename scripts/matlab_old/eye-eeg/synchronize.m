...
validDiffs = eegEventHasPartner(~isnan(eegEventHasPartner(:,end)), end);

% Counts within ±1 and ±4 samples
metrics.num_within_1smp = sum(abs(validDiffs) <= 1);
metrics.num_within_4smp = sum(abs(validDiffs) <= 4);
