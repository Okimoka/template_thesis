...
% Define a window of +/- 5 samples around the peak to exclude from noise
peakWindow = 5;
noiseIdx = abs(lags - sampleDiff) > peakWindow;
if any(noiseIdx)
    snr_value = abs(xc(ix)) / mean(abs(xc(noiseIdx)));
else
    snr_value = NaN;
end
