...
def save_logs(*, config: SimpleNamespace, logs: Iterable[pd.Series]) -> None:
    ...
    with FileLock(fname.with_suffix(fname.suffix + ".lock")):
        append = fname.exists()
        if append and not zipfile.is_zipfile(fname):
            timestamp = time.strftime("%Y%m%d-%H%M%S")
            corrupt_fname = fname.with_name(
                f"{fname.stem}.corrupt-{timestamp}{fname.suffix}"
            )
            fname.replace(corrupt_fname)
            msg = (
                "Detected a corrupted Excel log workbook. "
                f"Moved it aside to {corrupt_fname.name} and recreating the log file."
            )
        ...
        try:
            ...
            if fname.exists():
                fname.replace(corrupt_fname)
            msg = (
                "Encountered an unreadable Excel log workbook while opening it. "
                f"Moved it aside to {corrupt_fname.name} and recreating the log file."
            )
            logger.warning(**gen_log_kwargs(message=msg))
            writer = pd.ExcelWriter(
