...
def _get_bids_path_in(
    *,
    ...
    if kind != "orig":
        ...
        # Keep suffix explicit for raw BIDS data so run labels are matched
        # against the full "<...>_run-<N>_<suffix>" stem.
        # This avoids ambiguous run-1 vs run-10/run-11 matching behavior
        # with older mne-bids versions that use prefix globbing.
        path_kwargs["suffix"] = None #get_datatype(config=cfg)
        #print("suffix is " + str(get_datatype(config=cfg)))
