...
def get_serial_report_exec_params(
    *,
    exec_params: SimpleNamespace,
    step: str,
) -> SimpleNamespace:
    """Force shared report-writing stages to run serially."""
    if get_n_jobs(exec_params=exec_params) == 1:
        return exec_params

    serial_exec_params = SimpleNamespace(**vars(exec_params))
    serial_exec_params.n_jobs = 1
    logger.info(
        **gen_log_kwargs(
            message=f"{step} with n_jobs=1 to serialize shared report writes."
        )
    )
    return serial_exec_params
